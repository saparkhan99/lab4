...
int main(int argc, char **argv)
{
  ...
  target_pose.pose.position.x = target_pose.pose.position.x-0.14; /* Basically our target pose is the same as current,
  ...
  while (ros::ok()){
    ...
    // moves 1/10 of the 1.4 distance whne i = 0.
    for (int i = 0; i < 3; i++){
      if (i==1){
        target_pose.pose.position.x = target_pose.pose.position.x-0.28;
        // Move 2/10 of 1.4 distance

      } else if (i==2){
        target_pose.pose.position.x = target_pose.pose.position.x-0.98;
        // Move 7/10 of 1.4 distance 
